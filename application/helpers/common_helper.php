...
{
	$ci =& get_instance();
	$single_manual_reservation = $ci->db->query("SELECT members_count,price,guest_name,num_nights,num_rooms,room_id,reservation_id, start_date, end_date ,channel_id , reservation_code,status FROM `".RESERVATION."` WHERE ( (DATE_FORMAT(STR_TO_DATE(start_date,'%d/%m/%Y'),'%Y-%m-%d') >= '".$start_date."' AND DATE_FORMAT(STR_TO_DATE(start_date,'%d/%m/%Y'),'%Y-%m-%d') <= '".$end_date."') OR ( (DATE_FORMAT(STR_TO_DATE(end_date,'%d/%m/%Y'),'%Y-%m-%d') > '".$start_date."' AND DATE_FORMAT(STR_TO_DATE(end_date,'%d/%m/%Y'),'%Y-%m-%d') <= '".$end_date."') ) ) AND `reservation_id` = '".$get_reser."' AND user_id ='".current_user_type()."' AND hotel_id = '".hotel_id()."' AND room_id='".insep_decode($property_id)."' AND status NOT IN('Canceled') ORDER BY reservation_id DESC");
