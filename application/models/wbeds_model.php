...
class Wbeds_model extends CI_Model
{
	...
	{	
		...
		if($channel_id=='14')
		{
			...
			$this->db->select('WB.import_mapping_id, WB.nameRoomType, WB.codeRate, WB.nameRoomFeature , WB.boardCodeBase, WB.maximumPaxes, WB.supportSaleSystem, WB.boardCodeBase');
			if($clean!='')
			{
				$this->db->where_not_in('WB.import_mapping_id',$import);
			}
			$this->db->where(array('user_id'=>$owner_id,'hotel_id'=>hotel_id()));
			$result = $this->db->get(IM_WBEDS.' as WB');
			if($result!='')
			{
	...
	{	
		...
		if($channel_id=='14')
		{
			$count = $this->db->select('import_mapping_id')->from(IM_WBEDS)->where(array('user_id'=>current_user_type(),'hotel_id'=>hotel_id(),'HotelCode'=>$ch_details->hotel_channel_id))->count_all_results();
			return $count;
		}
	}
	...
	function get_all_mapped_rooms($channel_id)
	{
		...
		if($channel_id==14)
		{
			$this->db->join(IM_WBEDS.' as WB','R.import_mapping_id=WB.import_mapping_id');
			$this->db->join(CONNECT.' as C','WB.HotelCode=C.hotel_channel_id');
		}
	...
	function importAvailabilities($user_id = "", $hotel_id = "",$channel,$mapping,$import_mapping_id,$start_date = "",$end_date = "",$importRates='')
    {
        ...
        $ch_details = get_data(CONNECT,array('user_id'=>$user_id,'hotel_id'=>$hotel_id,'channel_id'=>"14",'status'=>'enabled'))->row();
		
		if($ch_details)
		{
			$channel_id = 14;                   
			
			$mp_details = get_data(IM_WBEDS,array('user_id'=>$user_id,'hotel_id'=>$hotel_id,'channel_id'=>$channel_id,'import_mapping_id'=>$import_mapping_id,'HotelCode'=>$ch_details->hotel_channel_id))->row();
						   ...
						   <soapenv:Body>
						      <impl:getInventoryRQ>
						         <impl:getInventoryRQRow>
						            ...
						            <impl:beginDate>'.$start_date.'</impl:beginDate>
						            <impl:endDate>'.$end_date.'</impl:endDate>
						            <impl:codeRate>'.$mp_details->codeRate.'</impl:codeRate>
						         </impl:getInventoryRQRow>
						      </impl:getInventoryRQ>
						   </soapenv:Body>
						</soapenv:Envelope>';
			...
			$x_r_rq_data['section'] 	= 	'WBEDS_IM_AV_REQ';
			...
			$url = "http://test.suppliers.gsisservices.com:7010/GServices/wsSuppliers";
			$headers = array(
			...
			$x_r_rs_data['channel_id'] 	= 	'14';						
			$x_r_rs_data['user_id'] 	= 	'0';						
			$x_r_rs_data['hotel_id'] 	= 	'0';						
			$x_r_rs_data['message'] 	= 	$response;						
			$x_r_rs_data['type'] 		= 	'RES';						
			$x_r_rs_data['section'] 	=	'WBEDS_IM_AV_RES';
			insert_data(ALL_XML,$x_r_rs_data);
			$Hotelarray = $responseArray['soapenvBody']['getInventoryRS']['getInventoryRSRow']['curOutInventoryRow'];
			$Errorarray = @$responseArray['soapenvBody']['getInventoryRS']['getInventoryRSRow']['Warning'];
			...
			{
				foreach($Hotelarray as $rooms)
				{
					if($mp_details->codeRoomType == (string)$rooms['codeRoomType'] 
						&& $mp_details->codeRoomFeature == (string)$rooms['codeRoomFeature'] 
						&& $mp_details->codeRate == (string)$rooms['codeRate']
						)
					{
						$date = (string)$rooms['dateInventory'];

						$sep_date = $date;

						if($importRates == "")
						{
							$allotment = (string)$rooms['maximumSales'];
						}else{	

							$allotment = (string)$rooms['boardPrice'];
						}
		...
		if($count!=0)
		{
			...
			if($room_mapping)
			{
				...
				foreach($room_mapping as $room_value)
				{
					...
					$mp_details = get_data(IM_WBEDS,array('user_id'=>current_user_type(),'hotel_id'=>hotel_id(),'channel_id'=> $channel_id,'import_mapping_id'=>$room_value->import_mapping_id))->row(); 
					...
					if($room_value->update_rate=='1' && @$product['price'] != "")
					{
						...
						$xml_price = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:impl="http://impl.wsSuppliers.integracion/">
							   <soapenv:Header/>
							   <soapenv:Body>
							      <impl:updatePriceRQ>
							         <impl:updatePriceRQRow>
							            <impl:idCliePor>'.$ch_details->user_name.'</impl:idCliePor>
							            <impl:pwCliePor>'.$ch_details->user_password.'</impl:pwCliePor>
							            <impl:ideCHM>HOTERATUS</impl:ideCHM>
							            <impl:codeRate>'.$mp_details->codeRate.'</impl:codeRate>
							            <impl:referenceHotel>'.$mp_details->HotelCode.'</impl:referenceHotel>
							            <impl:beginDate>'.$product['start_date'].'</impl:beginDate>
							            <impl:endDate>'.$product['end_date'].'</impl:endDate>
							            <impl:codeRoomType>'.$mp_details->codeRoomType.'</impl:codeRoomType>
							            <impl:codeRoomFeature>'.$mp_details->codeRoomFeature.'</impl:codeRoomFeature>
							            <impl:boardCode>'.$mp_details->boardCodeBase.'</impl:boardCode>
							            <impl:price>'.$product['price'].'</impl:price>
							         </impl:updatePriceRQRow>
							      </impl:updatePriceRQ>
							   </soapenv:Body>
							</soapenv:Envelope>';
						...
						if($C_URL)
						{
							...
							$headers = array(
								"Content-type: application/soap+xml; charset=utf-8",
								"Host:www.reconline.com",
								"Content-length: ".strlen($xml_price),
							); //SOAPAction: your op URL
							...
							curl_setopt($ch, CURLOPT_MAXREDIRS, 12);
							curl_setopt($ch, CURLOPT_POST, true);
							curl_setopt($ch, CURLOPT_POSTFIELDS, $xml_price); // the SOAP request
							curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
							$ss = curl_getinfo($ch);		        
							$response = curl_exec($ch);
							$xml = preg_replace("/(<\/?)(\w+):([^>]*>)/",'$1$2$3',$response);
							...
							$Hotelarray = $responseArray['soapenvBody']['updatePriceRS']['updatePriceRSRow']['error'];
							if((string)$Hotelarray != 'OK')
							{
								$this->inventory_model->store_error(current_user_type(),hotel_id(),$channel_id,(string)$Hotelarray,'Bulk Update',date('m/d/Y h:i:s a', time()));
								$this->session->set_flashdata('bulk_error',(string)$Hotelarray);
							}
						}
					...
					{
						$update_values = 'yes';
						$xml_availability = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:impl="http://impl.wsSuppliers.integracion/">
							   <soapenv:Header/>
							   <soapenv:Body>
							      <impl:updateInventoryRQ>
							         <impl:updateInventoryRQRow>
							            ...
							            <impl:codeRoomType>'.$mp_details->codeRoomType.'</impl:codeRoomType>
							            <impl:codeRoomFeature>'.$mp_details->codeRoomFeature.'</impl:codeRoomFeature>
							            <impl:typeInventory>L</impl:typeInventory>';
						if($stop_sale == 1)
						{
							$xml_availability .= '<impl:statusInventory>C</impl:statusInventory>';
						}else if($stop_sale == 'remove'){
							$xml_availability .= '<impl:statusInventory>A</impl:statusInventory>';
						}			
						if(@$product['availability'] != "")
						{
							$xml_availability .= '<impl:allotment>'.$product['availability'].'</impl:allotment>';
						}
						$xml_availability .= '</impl:updateInventoryRQRow>
							      </impl:updateInventoryRQ>
							   </soapenv:Body>
							</soapenv:Envelope>';
						...
						if($C_URL)
						{
							...
							$headers = array(
								"Content-type: application/soap+xml; charset=utf-8",
								"Host:www.reconline.com",
								"Content-length: ".strlen($xml_availability),
							); //SOAPAction: your op URL
							...
							curl_setopt($ch, CURLOPT_MAXREDIRS, 12);
							curl_setopt($ch, CURLOPT_POST, true);
							curl_setopt($ch, CURLOPT_POSTFIELDS, $xml_availability); // the SOAP request
							curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
							$ss = curl_getinfo($ch);		        
							$response = curl_exec($ch);
							$xml = preg_replace("/(<\/?)(\w+):([^>]*>)/",'$1$2$3',$response);
							...
							$Hotelarray = $responseArray['soapenvBody']['updateInventoryRS']['updateInventoryRSRow']['error'];
							if((string)$Hotelarray != 'OK')
							{
								$this->inventory_model->store_error(current_user_type(),hotel_id(),$channel_id,(string)$Hotelarray,'Bulk Update',date('m/d/Y h:i:s a', time()));
								$this->session->set_flashdata('bulk_error',(string)$Hotelarray);
					...
					{
						$xml_minimum = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:impl="http://impl.wsSuppliers.integracion/">
						   	<soapenv:Header/>
						   	<soapenv:Body>
						      <impl:setStayMinimumRQ>
						         <impl:setStayMinimumRQRow>
						            <impl:idCliePor>'.$ch_details->user_name.'</impl:idCliePor>
						            <impl:pwCliePor>'.$ch_details->user_password.'</impl:pwCliePor>
						            <impl:ideCHM>HOTERATUS</impl:ideCHM>
						            <impl:codeRate>'.$mp_details->codeRate.'</impl:codeRate>
						            ...
						            <impl:beginDate>'.$product['start_date'].'</impl:beginDate>
						            <impl:endDate>'.$product['end_date'].'</impl:endDate>
						            <impl:applyMonday>'.$bnow_mon.'</impl:applyMonday>
						            <impl:applyTuesday>'.$bnow_tue.'</impl:applyTuesday>
						            <impl:applyWednesday>'.$bnow_wed.'</impl:applyWednesday>
						            <impl:applyThursday>'.$bnow_thur.'</impl:applyThursday>
						            <impl:applyFriday>'.$bnow_fri.'</impl:applyFriday>
						            <impl:applySaturday>'.$bnow_sat.'</impl:applySaturday>
						            <impl:applySunday>'.$bnow_sun.'</impl:applySunday>
						            <impl:typeApplication>P</impl:typeApplication>
						            <impl:typeApplicationDay>L</impl:typeApplicationDay>
						            <impl:numberNights>'.$product['minimum_stay'].'</impl:numberNights>
						            <impl:codeRoomType>'.$mp_details->codeRoomType.'</impl:codeRoomType>
						            <impl:codeRoomFeature>'.$mp_details->codeRoomFeature.'</impl:codeRoomFeature>
						            <impl:boardCode>'.$mp_details->boardCodeBase.'</impl:boardCode>
						            <impl:onRequest>N</impl:onRequest>
						         </impl:setStayMinimumRQRow>
						      </impl:setStayMinimumRQ>
						    </soapenv:Body>
						</soapenv:Envelope>';
						...
						if($C_URL)
						{
							...
							echo "<pre>";
							echo $responseArray;
							$Hotelarray = $responseArray['soapenvBody']['setStayMinimumRS']['setStayMinimumRSRow']['error'];
							if((string)$Hotelarray != 'OK')
							{
								$this->inventory_model->store_error(current_user_type(),hotel_id(),$channel_id,(string)$Hotelarray,'Bulk Update',date('m/d/Y h:i:s a', time()));
								$this->session->set_flashdata('bulk_error',(string)$Hotelarray);
						...
						if($room_mapping)
						{
							...
							foreach($room_mapping as $room_value)
							{
										   ...
										   <soapenv:Body>
										      <impl:updatePriceRQ>
										         <impl:updatePriceRQRow>
										            ...
										            <impl:codeRate>'.$mp_details->codeRate.'</impl:codeRate>
										            <impl:referenceHotel>'.$mp_details->HotelCode.'</impl:referenceHotel>
										            <impl:beginDate>'.$value['start_date'].'</impl:beginDate>
										            <impl:endDate>'.$value['end_date'].'</impl:endDate>
										            <impl:codeRoomType>'.$mp_details->codeRoomType.'</impl:codeRoomType>
										            <impl:codeRoomFeature>'.$mp_details->codeRoomFeature.'</impl:codeRoomFeature>
										            <impl:boardCode>'.$mp_details->boardCodeBase.'</impl:boardCode>
										            <impl:price>'.$value_price.'</impl:price>
										         </impl:updatePriceRQRow>
										      </impl:updatePriceRQ>
										   </soapenv:Body>
										</soapenv:Envelope>';
								...
								if($room_value->update_availability == '1')
								{
									...
									{
										$xml_availability .= '<impl:allotment>'.$value['availability'].'</impl:allotment>';
									}
									$xml_availability .= '</impl:updateInventoryRQRow>
										      </impl:updateInventoryRQ>
										   </soapenv:Body>
		...
		if($count!=0)
		{
			if($table=='room_update')
			{
				if($column != "availability"){
					...
					$room_mapping = get_data(MAP,array('owner_id'=>current_user_type(),'hotel_id'=>hotel_id(),'channel_id'=>$con_ch,'property_id'=>$room_id,'enabled'=>'enabled','update_rate'=>1))->result();
				}
			}
	...
	function fullUpdate($source,$period)
	{
		...
		if(count($all_map_rooms)!=0)
		{
            ...
            {   
				...
				foreach($period as $date)
                {
					$separate_date = date('Y-m-d',strtotime(str_replace('/','-',$date->format("d/m/Y"))));
					
					if($room_value->property_id!='0' && $room_value->rate_id=='0' && $room_value->guest_count=='0' && $room_value->refun_type=='0')
					{
						$update_details = get_data(TBL_UPDATE,array('owner_id'=>current_user_type(),'hotel_id'=>hotel_id(),'room_id'=>$room_value->property_id,'individual_channel_id'=>$individual_channel_id,'separate_date'=>$date->format("d/m/Y")))->row();
					...
					{
						$update_details = get_data(RESERV,array('owner_id'=>current_user_type(),'hotel_id'=>hotel_id(),'room_id'=>$room_value->property_id,'individual_channel_id'=>$individual_channel_id,'separate_date'=>$date->format("d/m/Y"),'guest_count'=>$room_value->guest_count,'refun_type'=>$room_value->refun_type))->row();
					...
					elseif($room_value->property_id!='0' && $room_value->rate_id!='0' && $room_value->guest_count=='0' && $room_value->refun_type=='0')
					{
						$update_details = get_data(RATE_BASE,array('owner_id'=>current_user_type(),'hotel_id'=>hotel_id(),'room_id'=>$room_value->property_id,'rate_types_id'=>$room_value->rate_id,'individual_channel_id'=>$individual_channel_id,'separate_date'=>$date->format("d/m/Y")))->row();
					...
					elseif($room_value->property_id!='0' && $room_value->rate_id!='0' && $room_value->guest_count!='0' && $room_value->refun_type!='0')
					{
						$update_details = get_data(RATE_ADD,array('owner_id'=>current_user_type(),'hotel_id'=>hotel_id(),'room_id'=>$room_value->property_id,'rate_types_id'=>$room_value->rate_id,'individual_channel_id'=>$individual_channel_id,'separate_date'=>$date->format('d/m/Y'),'guest_count'=>$room_value->guest_count,'refun_type'=>$room_value->refun_type))->row();
