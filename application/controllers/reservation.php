...
function save_reservation()
{
	...
	else if($this->input->get('payment_type')=='cc')
	{
		if($this->input->get('use_existing_card')=='true')
		{
			$user_details 	=	get_data(TBL_USERS,array('user_id'=>current_user_type()))->row();
			
			$card_details 	=	get_data(TBL_CREDIT,array('id'=>$this->input->get('existing_card')))->row();
			...
			$paymentmethod	=   (get_data(CCTYPES,array('cc_type_id'=>$this->input->get('card_type')))->row()->seo_url);
			...
			$price          =   $this->input->get('oringinal_price');
		}
		elseif($this->input->get('use_existing_card')=='false' || $this->input->get('use_existing_card')=='')
		{
			$cdata['user_id']		=	current_user_type();
			$cdata['c_fname']       = 	(string)safe_b64encode($this->input->get('card_name'));
			$cdata['c_lname']       = 	'';//(string)safe_b64encode($this->input->get('card_name'));
			$cdata['card_number']   = 	(string)safe_b64encode($this->input->get('card_number'));
			$cdata['exp_month']     = 	(string)safe_b64encode($this->input->get('exp_month'));
			$cdata['exp_year']      =	(string)safe_b64encode($this->input->get('exp_year'));
			$cdata['cvv']           = 	(string)safe_b64encode($this->input->get('security_code'));
			$cdata['card_type']     = 	(string)safe_b64encode(get_data(CCTYPES,array('cc_type_id'=>$this->input->get('card_type')))->row()->seo_url);//$payment_card;
		...
		if(($paymentmethod=="visa") || ($paymentmethod=="american-express") || ($paymentmethod=="mastercard") || ($paymentmethod=="discover") )  
		{        
		   ...
		   /*  echo '<pre>';
			...
			{                   
				...
				{                    
					...
					/* $results        =	$this->inventory_model->updatetransaction(insep_decode($subscribe_plan_id),
					
					$transaction_id,$connect_channels); */
					/* echo $results; die; */
					/* if($results)
					{
				...
				{   
					...
					$pay_error['error']		=	$this->authorize_net->getError();
					
					//$this->session->set_flashdata('error',"Invalid card number");
					
					//$this->session->set_flashdata('pay_error',$this->authorize_net->getError());
			
					//echo $this->authorize_net->getError();                             
				}
			}   
			else
			{   
		...
		else
		{
			...
			$pay_error['error']		=	"Please try another card or other payment!!!";
			
			//$this->session->set_flashdata('pay_error',"Please try another card or other payment!!!");
		}
		/* echo 'cc';
		echo '<pre>';
		print_r($this->input->get());
	...
	else if($this->input->get('payment_type')=='pp')
	{
		...
		<input type="hidden" name="item_name" value="<?php echo 'New Reservation';?>">
		...
		<input type="hidden" name="return" value="<?php echo lang_url().'reservation/ReservationPayment_Success'; ?>">
		

		 <input type="hidden" name="custom" value="<?php echo $first_name.'|'.$email.'|'.$hotel_id.'|'.$user_id.'|'.$reservation_code.'|'.$num_rooms.'|'.$child.'|'.$start_date.'|'.$end_date.'|'.$phone.'|'.$room_id.'|'.$rate_type_id.'|'.$nights.'|'.$rooms.'|'.$adult.'|'.$street_name.'|'.$country.'|'.$province.'|'.$city_name.'|'.$zipcode.'|'.$booking_date.'|'.$notes.'|'.$price_details.'|'.$currency.'|'.$guestmail?>"> 

		<input type="submit" id="pay_now" style="display:none;">
		</form> 
		<center>
		<div id="preloaders" style="display:block !important;">
		Loading Please wait....
		</div>
		</center>
   <?php }      
	else if($this->input->get('payment_type')=='bt')
	{
	/*	echo 'bt';
		echo '<pre>';
		print_r($this->input->get());
        ...
        $bank_id      = $_GET['bank_type'];
         ...
         if($save_reserve)
			...
			else
			{
				...
				$pay_error['error']		=	"Error occurred Your Transaction!!!";
			}

         /*if($update_bankdetails!='')
         {

         	$admin_detail = get_data(TBL_SITE,array('id'=>1))->row();
         	$get_email_info     =   get_mail_template('22');

	        $subject            =   $get_email_info['subject'];

	        $template           =   $get_email_info['message'];

	        $refer = $reference_code;

	        $bank_details = json_decode($bank_deta);
	...
	if(admin_id()!='' && admin_type()=='1')
	{
		...
		if($passwod == $_REQUEST['password'])
		{
			...
			else if($channel==1)
			{
				...
				if(strlen($reservation_channeldetails['SeriesCode'])>1)
				{
					$data['URL'] = lang_url()."reservation/Showcvc/".safe_b64encode($reservation_channeldetails['SeriesCode']);
				}
				else
				{
			...
			else if($channel==2)
			{
				...
				if (strlen(safe_b64decode($booking_table['cc_number']))>5)
				{
					...
					$data['CC_NUMBER']			=	'Press button to see';
				}
				else
				{
					$data['URL'] = lang_url()."reservation/Showcvc/".safe_b64encode("Does Not Have");
...
received">
	...
	function getResrvationSummery($channel_id)
	{
		...
		if(insep_decode($channel_id) == 2) 
		{
			...
			curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
			curl_setopt($ch, CURLOPT_POST, 1);
			curl_setopt($ch, CURLOPT_HTTPHEADER,  array('X-CC-URL: https://secure-supply-xml.booking.com/hotels/xml/reservationssummary',"X-CC-SIGN: 170426055204280709","X-CC-MERCHANT-ID: 3000011638",'Content-Type: text/xml'  ));
			curl_setopt($ch, CURLOPT_POSTFIELDS, $xml_data);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			$output = curl_exec($ch);
			$x_r_rs_data['channel_id'] = '2';						
	...
	function getResrvationCronFromBooking($channel_id='')
	{
...

		{
			...
			foreach ($AllConections as  $value) {
		        ...
		        if (count($data_api)>0)
		       {
		       	$this->saveBooking_Booking_Cron($data_api,$ruid,$cha_name="Booking.com",$hotel_channel_id='',$x_r_rs_data);
		       	mail("datahernandez@gmail.com"," Reservaciones importadas".count($data_api),count($data_api).' '.$property_name);
				}
