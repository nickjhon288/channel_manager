...
class Format {
	 ...
	 * @return  Factory
	 */
	public function factory($data, $from_type = null,$keyName = 'item')
	{
		// Stupid stuff to emulate the "new static()" stuff in this libraries PHP 5.3 equivilent
		$class = __CLASS__;
		return new $class($data, $from_type,$keyName);
	}

	/**
	 * Do not use this directly, call factory()
	 */
	public function __construct($data = null, $from_type = null, $keyName = 'item')
	{
	...
	public function to_xml($data = null, $structure = null, $basenode = 'Response',$attribute = "")
	{
		if ($data === null and ! func_num_args())
		{
			$data = $this->_data;
		...
		foreach ($data as $key => $value)
		{
			...
			if (is_numeric($key))
            {
                ...
                $key = (singular($basenode) != $basenode) ? singular($basenode) : $this->_key_name;
                //$key = $this->_key_name;
            }
            ...
            if (is_array($value) || is_object($value))
            {
            	if(strpos($key, 'day=') !== FALSE){
            		$org_key = explode(' ', $key);
            		$node = $structure->addChild($org_key[0]);
            		$key_val = explode('=', $org_key[1]);
            		$node->addAttribute($key_val[0],trim($key_val[1],'"'));

            		$this->to_xml($value, $node, $org_key[0]);
            	}elseif($key != "attributes")
            	{
            		$node = $structure->addChild($key);
            		$attr = "";
            		$this->to_xml($value, $node, $key ,$attr);
            	}else{
            		$node = $structure;
            		$attr = "attribute";
            		foreach ($value as $key => $val) {
            			if(!is_array($val))
            			{
            				$structure->addAttribute($key, $val);            				
            			}else{
            				foreach($val as $k => $v)
            				$structure->addAttribute($k, $v);
            			}
            		}
            	}
            }
