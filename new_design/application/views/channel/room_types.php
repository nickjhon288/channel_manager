...
<div class="pa-n nn2">
    ...
    <a href="<?php echo lang_url();?>channel/manage_rooms">My Property</a>
 ...
 <div class="col-md-8">
    ...
    <select id="room_types" name="room_types" class="form-control" onchange="return room_types(this.value);">
...
<div class="col-md-8 col-sm-8 ssw vi1">
  <div class="form-group">
    <input type="number" value="<?php echo $existing_room_count;?>" step="any" class="form-control widh" name="existing_room_count" id="existing_room_counts" min="1">
  </div>
</div>
</div>
...
<a href="javascript:;" data-keyboard="false" data-backdrop="static" data-target="#myModal-tab-1" data-toggle="modal"><i class="fa fa-genderless"></i> <?php echo $roo;?></a>
<?php }
elseif($roo=='Select amenities'){  ?>
<a href="#tab_default_4" data-toggle="tab"><i class="fa fa-genderless"></i> <?php echo $roo;?></a>
<?php } else if($roo=='Upload photos'){?>
<a href="#tab_default_6" data-toggle="tab"><i class="fa fa-genderless"></i> <?php echo $roo;?></a>
<?php } ?>
...
<div class="col-md-4 col-sm-4 mar-top10"><span class="ne-k">Base price / Per night</span></div>
<div class="col-md-8 col-sm-8 mar-top10">
...
/* if(user_type()=='1' || admin_id()!='' && admin_type()=='1'){
	$get_data = get_data(HOTEL,array('owner_id'=>user_id(),'hotel_id'=>hotel_id()))->row()->subscribe_status==0;
   }
   else if(user_type()=='2'){
	$get_data = get_data(HOTEL,array('owner_id'=>owner_id(),'hotel_id'=>hotel_id()))->row()->subscribe_status==0;   
   }
...
<?php /* } else { */ ?>
<?php  if(user_type()=='1' || user_type()=='2' && in_array('4',user_edit()) || admin_id()!='' && admin_type()=='1'){ ?>
<a class="btn btn-primary" href="javascript:;" data-target="#add_rate" data-toggle="modal" data-backdrop="static" data-keyboard="false"><i class="fa fa-plus"></i>  Add rate type</a>
<?php } ?>
<?php /* } */ ?>
</center></div>
</div>
        ...
        <div class="table-wrapper orders-table section">
          ...
          <div>
            <div class="row filter-block">
			  ...
			  <?php  if(user_type()=='1' || user_type()=='2' && in_array('4',user_edit()) || admin_id()!='' && admin_type()=='1'){ ?>
                <a id="new-rate"  class="btn btn-success" href="javascript:;" data-target="#add_rate" data-toggle="modal" data-backdrop="static" data-keyboard="false"><i class="fa fa-plus"></i> Add rate type</a>
			  <?php } ?>
              </div>
            </div>
            ...
            <div class="row">
              <table class="table table-hover" id="original-container">
                ...
                <tr class="item" id="hr_variant_rate_<?php echo insep_encode($uniq_id);?>" style="">
                
  					<td class="handle"><a data-remote="true" href="<?php echo lang_url();?>inventory/manage_rate_types/edit/<?php echo $uniq_id;?>">
					<?php  if($rate_name!=''){ echo $rate_name; } else { echo '#'.$uniq_id;}?></a>
                      <br>
                    ...
                    <td class="handle"> <?php if($droc=='1') { ?>Yes <?php } else { echo 'No';}?></td>	
					<?php  if(user_type()=='1' || user_type()=='2' && in_array('4',user_edit()) || admin_id()!='' && admin_type()=='1'){ ?>
                    <td class="handle"><a href="javascript:;" title="Delete this rate type"class="del_rate" data-id="<?php echo insep_encode($uniq_id);?>" room-id="<?php echo insep_encode($room_id);?>"> <i class="fa fa-trash-o text-danger"> </i></a></td>	
					<?php }
					   else {?>
						   <td class="handle"><a href="javascript:;"> <i class="fa fa-trash-o text-danger"> </i></a></td>	
					<?php }
					?>
				</tr>
                <?php } ?>
...
<?php 
	...
	<div class="col-md-3 col-sm-3">
		...
		<a class="sser" href="<?php echo lang_url(); ?>mapping/settings/<?php echo insep_encode($channels->channel_id);?>"><div class="ss2">
		
		<img class="img img-responsive" src="data:image/png;base64,<?php echo base64_encode(file_get_contents("uploads/".$channels->image));?>">
		<h4 class="text-center"><i class="fa fa-link"></i> <?php echo $channels->channel_name;?></h4></div>
		</a>
	</div>
	...
	<?php } } } else { ?>
		...
		<div class="res-p"><center><a class="btn btn-primary" href="<?php echo lang_url();?>channel/channel_listing"><i class="fa fa-plus"></i>  Map Channel</a></center></div>
		<?php } ?>
		</div>
...
<input class="btn btn-success" type="submit" value="Upload" id="uploadButton" disabled="disabled">
    ...
    <div class="modal-content" id="success_msg">
     ...
     <div class="row">
	  ...
	  <div class="row">
		 ...
		 <div class="col-md-8 col-sm-8"> <input type="text" placeholder="Rate Type Name" id="property_name" name="property_name" class="form-control"></div>
		 
      </div>
 <?php $room_detial = get_data(TBL_PROPERTY,array('property_id'=>$property_id))->row();?>
 	  <div class="row">
 <div class="col-md-3 col-sm-3"><span class="ne-n-pad">Pricing Type <?php //echo $room_detial->pricing_type;?><span class="errors"> </span></span></div>
 <div class="col-md-8 col-sm-8 mar-top7">
	...
	?>
      ...
      <input type="hidden" name="base_price" id="base_price" value="<?php echo get_data(TBL_PROPERTY,array('property_id'=>$property_id))->row()->price;?>"/>
  	  ...
  	  <div class="col-md-12 guest_based <?php if($room_detial->pricing_type==1) { ?> display_none <?php } ?>">
	  ...
	  <div class="col-md-12 room_based <?php if($room_detial->pricing_type==2) { ?> display_none <?php } ?>">
      <div class="table table-responsive">
        <table class="table table-condensed">
        <thead>
        <tr>
