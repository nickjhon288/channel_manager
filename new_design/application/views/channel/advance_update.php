...
<div class="pa-n"><h4><a href="<?php echo lang_url();?>inventory/advance_update">Calendar</a>
    <i class="fa fa-angle-right"></i>
    Advanced Updates <i class="fa fa-angle-right"></i> <?php if($action!='reservation_no')
{ echo 'Reservation Yes'; } else { echo 'Reservation No';}?>
    </h4></div>
</div>
</div>
</div>
...
$all_room = get_data(TBL_PROPERTY,array('owner_id'=>user_id(),'droc'=>'1'))->result_array();
if(count($all_room)!=0) {
	?>
...
<span title="Price"><?php echo get_data(TBL_CUR,array('currency_id'=>$currency))->row()->symbol;?></span>
...
<div class="g-top-n ji pull-left" style="clear:both; overflow:hidden; text-align:center !important;" align="center">
<span title="Rooms still available"> R </span>
<a href="javascript:;" id="inline_username" class="inline_username" data-type="number" data-name="availability" data-pk="<?php echo $ss_da.'-'.$property_id;?>" data-url="<?php echo lang_url()?>inventory/inline_edit" data-title="Change Availability" title="Rooms still available"><?php echo ($single->availability); ?> </a>
</div>

<div class="g-top-n ji pull-left" style="clear:both; overflow:hidden; text-align:center !important;" align="center">
<span title="Minimum Stay"> M </span>
...
<li class="orgs"><a class="btn btn-default" href="<?php echo lang_url();?>inventory/bulk_update/reservation_no" role="button">Bulk Update</a></li>
</ul>
...
<form method="post" action="<?php echo lang_url();?>inventory/reservation_update_no" class="master_calendar_form">
...
<div class="g-top-n ji pull-left" style="clear:both; overflow:hidden; text-align:center !important;" align="center">
<a href="javascript:;" id="inline_username" class="inline_username pull-right" data-type="number" data-name="price" data-pk="<?php echo $ss_da.'-'.$property_id;?>" data-url="<?php echo lang_url()?>inventory/inline_edit_no" data-title="Update Price"><span title="Click to update the price"></span><i class="fa fa-money" title="Click to update the price"></i></a>
</div>

<div class="g-top-n ji pull-left" style="clear:both; overflow:hidden; text-align:center !important;" align="center">
<a href="javascript:;" id="inline_username" class="inline_username pull-right" data-type="number" data-name="availability" data-pk="<?php echo $ss_da.'-'.$property_id;?>" data-url="<?php echo lang_url()?>inventory/inline_edit_no" data-title="Update Availability" style="font-size:15px"><span title="Click to update the availability"></span><i class="fa fa-home" title="Click to update the availability"></i></a>
</div>
<div class="g-top-n ji pull-left cal_ms" style="clear:both; overflow:hidden; text-align:center !important;" align="center">
<a href="javascript:;" id="inline_username" class="inline_username pull-right" data-type="number" data-name="minimum_stay" data-pk="<?php echo $ss_da.'-'.$property_id;?>" data-url="<?php echo lang_url()?>inventory/inline_edit_no" data-title="Update Minimustay"><span title="Click to update the minimum stay"></span><i class="fa fa-moon-o" title="Click to update the minimum stay"></i></a>
</div>
...
<td class="<?php echo $color?> tal_td_bor">
<div class="g-top-n ji pull-left">
<a href="javascript:;" id="inline_username" class="inline_username pull-right" data-type="number" data-name="price" data-pk="<?php echo $ss_da.'-'.$property_id.','.$v.'~'.$refun;?>" data-url="<?php echo lang_url()?>inventory/inline_edit_guest" data-title="Update Price"><span title="Click to update the price"></span>&nbsp;<i class="fa fa-money" title="Click to update the price"></i></a>
</div></td>
<?php } else { ?> <td class="<?php echo $color?> tal_td_bor">
<div class="g-top-n ji pull-left">
<a href="javascript:;" id="inline_username" class="inline_username pull-right" data-type="number" data-name="price" data-pk="<?php echo $ss_da.'-'.$property_id.','.$v.'~'.$refun;?>" data-url="<?php echo lang_url()?>inventory/inline_edit_guest" data-title="Update Price"><span title="Click to update the price"></span>&nbsp;<i class="fa fa-money" title="Click to update the price"></i></a>
...
<div class="checkbox">
  <label>
    ...
    <?php foreach($con_cha as $connected){ extract($connected);?>
<div class="checkbox">
  <label>
...
<div class="checkbox">
  <label>
    <input type="checkbox" value="" checked id="reser_yes_days">
    All
  </label>
</div>
</div>
...
<?php 
  ...
  <div class="checkbox">
    <label>
      <input type="checkbox" checked name="days[]" value="<?php echo $values[$key]?>" class="reser_yes_days"><?php echo $day?> 
    </label>
  </div>
  <?php } ?>
</div>
...
Channel Manager  :
Provide the link on Manage Contacts on admin panel - Fixed.

Mail Function - Fixed.

If new user login restrict the bulk update , Tax Categories , Policies , Payment Method , Reservation Yes & No section - Fixed

Reservationlisting - Fixed.

Provide the link on Manage Rooms on front end. - Fixed

Mange Credit cards - Fixed.	

Manger Users - Fixed.

Restrict the room display on clandar ( Yes / No ) - Fixed.

Remove alert message on master rate - FIXED.

Setiing the Cron on expiry the subscribe packaes ( Functionality ).

Subscribe plan redirect not working corectly - FIXED..

Set the loader on ajax functions.

Issues on display months for calendar.

Change the close icons on modal eindow & notification message.

Set the 404 page ( Functionality )

Issues on Customized Calender reservation no.
...
Analyze and work on overview calender based on hotel runner.
If Client Comments received.
If demo changes or bugs.
Analyze & implement the API's.
EBAY DETAILS:
...
Client comments , File inetgration , analyze and implement the work on each channel calendar.

Show the reservation details on main calendar.

Calendar section for reservation yes.

Client comments & calendar section for reservation yes.

Complete the multi property section in front end.
