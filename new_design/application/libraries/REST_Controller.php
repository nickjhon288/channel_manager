...
class REST_Controller extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->_xml_parse = TRUE;
        $xmlData = simplexml_load_string($resp);
        if ($xmlData === false) {
        	foreach(libxml_get_errors() as $error) {
        		$this->_xml_parse = FALSE;
        		$this->_errormesage = $error->message;
        	}
        }
		...
		if (config_item('rest_enable_keys'))
		{
			...
			// Start Gayathri //
			if($this->_allow === TRUE){
				if (config_item('rest_enable_password'))
				{
					$this->_pass_res = $this->_detect_password();
				}
			}
			// End Gayathri //
		}
	...
	public function _remap($object_called, $arguments)
	{
		...
		if($this->_xml_parse == FALSE)
		{
			$this->response(array('status' => 'false', 'error' => $this->_errormesage), 403);
		}
		...
		if (config_item('rest_enable_password'))
		{
			if ($this->_pass_res === FALSE AND $this->_allow === TRUE)
			{
				$this->response(array('status' => false, 'error' => 'Invalid Password.'), 403);
			}
		}
		// End Gayathri //
	 ...
	 */
	public function response($data = array(), $http_code = null,$keyName = "item")
	{
		...
		else
		{
			...
			elseif (method_exists($this->format, 'to_'.$this->response->format))
			{
				...
				$output = $this->format->factory($data,null,$keyName)->{'to_'.$this->response->format}();
			}
	...
	protected function _detect_api_key()
	{
        ...
        $xmlData = simplexml_load_string($resp);
		// Find the key from server or arguments
		if(isset($xmlData->ApiKey)){
			if($xmlData->ApiKey != ""){
				$key = (string)$xmlData->ApiKey;
				if ( ! $row = $this->rest->db->where('apikey', $key)->get(config_item('rest_keys_table'))->row())
				{
					return FALSE;
				}
	...
	protected function _detect_password()
	{
        ...
        $xmlData = simplexml_load_string($resp);
		// Find the key from server or arguments
		if(isset($xmlData->ApiKey) && isset($xmlData->Password)){
			$key = (string)$xmlData->ApiKey;
			$password = (string)$xmlData->Password;
			if ( ! $row = $this->rest->db->where(array('apikey'=>$key,'password' => $password))->get(config_item('rest_keys_table'))->row())
			{
				return FALSE;
			}
			$this->rest->key = $row->apikey;
