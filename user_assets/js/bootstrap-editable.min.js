...
! function(a) {
    ...
    b.prototype = {
        ...
        save: function(b) {
			...
			var custom_name = 	"function" == typeof this.options.name ? this.options.name.call(this.options.scope) : this.options.name;

			var result 		= 	d.split('-');

			var result_sub 	= 	result[1].split('~');
			
			var date_format = 	result[0].split('/');
			
			if(custom_name=='availability' && result_sub.length==1)
			{
				localStorage.name 			= 	custom_name;
				
				localStorage.main 			= 	result_sub[0];
				
				localStorage.date			= 	result[0].replace('/','_');
				
				localStorage.date_format	=	date_format[2]+'-'+date_format[1]+'-'+date_format[0];	 
        ...
        setValue: function(a, b) {
			if(localStorage.name=='availability')
			{
				$('.'+localStorage.date.replace('/','_')+'_'+localStorage.main).html(a);
				/* console.log(a);
				console.log(localStorage.name);
				console.log(localStorage.date);
				console.log(localStorage.main);
				console.log(localStorage.date_format);*/	
				
				var d		=	new Date(localStorage.date_format);
				var days 	= 	["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];
				var day 	= 	days[d.getDay(localStorage.date_format)];
				/* console.log(day); */
				if((day=="Saturday" || day=="Sunday") && a > 0)
				{
					if($('#'+localStorage.date.replace('/','_')+'_'+localStorage.main+'_M').is(':checked'))
					{
						$('.'+localStorage.date.replace('/','_')+'_'+localStorage.main+'_M').attr('bgcolor','#CC99FF');
					}
					else
					{
						$('.'+localStorage.date.replace('/','_')+'_'+localStorage.main+'_M').attr('bgcolor','#669933');
					}
					$('.'+localStorage.date.replace('/','_')+'_'+localStorage.main+'_R').attr('bgcolor','#669933');
				}
				else if(a > 0)
				{
					if($('#'+localStorage.date.replace('/','_')+'_'+localStorage.main+'_M').is(':checked'))
					...
					{
						$('.'+localStorage.date.replace('/','_')+'_'+localStorage.main+'_M').attr('bgcolor','#D9E4C3');
					}
					$('.'+localStorage.date.replace('/','_')+'_'+localStorage.main+'_R').attr('bgcolor','#D9E4C3');
				}
				else if( a < 0 )
				{
					$('.'+localStorage.date.replace('/','_')+'_'+localStorage.main+'_M').attr('bgcolor','#F4C327');
					$('.'+localStorage.date.replace('/','_')+'_'+localStorage.main+'_R').attr('bgcolor','#F4C327');
				}
				else if( a == 0 )
				{
					$('.'+localStorage.date.replace('/','_')+'_'+localStorage.main+'_M').attr('bgcolor','#FF0000');
					$('.'+localStorage.date.replace('/','_')+'_'+localStorage.main+'_R').attr('bgcolor','#FF0000');
				}
				localStorage.name = '';
				localStorage.main = '';
				localStorage.date = '';
				localStorage.date_format = '';
			}
